...
次はリポジトリに入っていない。**無いことを前提に、代わりの手段で進めること。**

| 必要なもの | 用途と代替手段 |
|---|---|
| **環境変数の実値** | 起動に必須。キー名は `.env.example` にある。値は引き渡し元に確認する |
| **オペレーターのログイン情報** | 管理画面に入るために必要。無ければ Supabase で新規作成できる（手順は `docs/manual-developer.md`） |
| **検証スクリプト** | 受入確認の自動化に使っていたもの。無くても `docs/test-scenarios.md` の期待値で手動確認できる |

> 上記が揃わなくても、コードの変更・型チェック・Lint・ビルドは実行できる。
> **「ファイルが無い」で止まらず、できるところまで進めて、
> 何が無くて何ができなかったかを報告すること。**
