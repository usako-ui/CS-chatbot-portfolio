...
| メニュー | 表示される内容 |
|---|---|
| 問い合わせ一覧 | **完了分も含めた全部**（絞り込みなし） |
| 新規 | まだ誰も返信していないもの（**ここから対応します**） |
| 対応中 | 誰かが返信を始めたもの |
| 完了 | 対応が終わったもの（自動完了したものも含む） |
| すべての会話 | 問い合わせ一覧と同じ（完了分も含めた全部） |

> 「問い合わせ一覧」と「すべての会話」は同じ内容が表示されます。
> **未対応のものだけを見たいときは「新規」を使ってください。**
...
完了にすると「完了」バッジが付き、左メニューの「完了」からも見られるようになります。
**問い合わせ一覧からは消えません**（一覧は絞り込みをしていないため）。
未対応のものだけを見たいときは「新規」を開いてください。
...
- 会話をクリックすれば、お客様の質問とAIの回答をそのまま読めます
- 自動完了した会話は**一覧の上部に表示されます**（最後に更新されたものが上に来るため）

休日明けに管理画面を開いたとき、休みの間に自動完了したものも一覧に並びます。
見落とすことはありません。
...
「新規」を開いている場合は、担当者が返信済み（対応中）か完了になっている可能性があります。
左メニューの「問い合わせ一覧」を開くと、完了分も含めて全部表示されます。
