...
| 目的 | 読む場所 | 所要 |
|---|---|---|
| **まず動かしてみる** | [はじめに：ローカルで動かす](#はじめにローカルで動かす) | 30分 |
...
> **初めてこのリポジトリを触る方は「はじめに」から順に進めてください。**
> 動く状態を作ってから読むほうが理解が早く、どこを直せばよいかも掴めます。
>
...
**まずここを通してください。** 動く状態を作ってからコードを読むほうが、はるかに早く理解できます。

### 必要なもの

| 必要なもの | 補足 |
|---|---|
| Node.js 20 以上 | `node -v` で確認。Next.js 15 の要件は 18.18 以上 |
| Supabase のアカウント | 無料プランで足ります |
| Google AI Studio の APIキー | <https://aistudio.google.com/apikey> で取得（無料枠あり） |
...
<https://supabase.com> で新規プロジェクトを作成します。作成後、
Project Settings → API から次の3つを控えます。

- Project URL
- `anon` key（公開してよいキー）
- `service_role` key（**絶対に公開してはいけないキー**）

**3. テーブルとRLSを作る**

`requirements.md` の「DBスキーマ」章にある DDL を、Supabase の
SQL Editor に貼り付けて実行します。テーブル4つ・RLSポリシー7つが作られます。

**4. FAQの初期データを入れる**

`supabase/seed.sql` を SQL Editor に貼り付けて実行します（18件入ります）。

> **FAQが0件だとAIは回答できません。** 根拠が無い状態でAIを呼ぶと
> 一般知識で答え始めるリスクがあるため、0件のときは
> エラーにして止める作りになっています（`lib/faq.ts`）。

**5. Realtime を有効にする（忘れると無言で壊れます）**
...
> ⚠️ **これを忘れると、購読は成功するのに何も届きません。**
> エラーも警告も出ないため、「返信しても顧客画面に出ない」という
> 症状だけが残り、原因に辿り着くのに時間を溶かします。

**6. 匿名サインインを有効にする（忘れると顧客側が動きません）**

```
Authentication → Providers → Anonymous を有効化
```

> ⚠️ **未設定だと匿名サインインが 422 エラーになり、チャットが全く動きません。**
...
Authentication → Users → Add user
  - Email / Password を入力
  - 「Auto Confirm User」を必ずチェック（確認メールを踏まずにログインできる）
```

作成後、そのユーザーの **User Metadata** に次を設定します。

```json
{ "display_name": "田中 美咲", "role_label": "フルタイム" }
```

> `display_name` が無いと管理画面の担当者名がメールアドレスのまま表示されます
> （`lib/operatorData.ts`）。動作はしますが見た目が業務向けになりません。
> 担当者の自動割り当てを試すなら**2アカウント**作ってください。
...
`.env.local` を開き、2で控えた値と Gemini のAPIキーを設定します。
各キーの意味は[5章](#5-環境変数一覧)を参照してください。
...
| # | 確認 | 期待する結果 |
|---|---|---|
| 1 | トップページ（`/`）を開く | 紹介ページが表示される |
| 2 | 管理画面（`/login`）でログイン | 問い合わせ一覧が表示される |
| 3 | 顧客チャットで「送料はいくらですか」と送る | 「送料は全国一律550円です…」と返る |
| 4 | 3の会話を管理画面から開いて返信 | **顧客側の画面に1秒ほどで表示される** |

**4がうまくいかない場合は、手順5（Realtime）を確認してください。**

### つまずきやすい点

| 症状 | 原因 |
|---|---|
| チャットが開かない・422エラー | 手順6（匿名サインイン）が未設定 |
| 返信しても顧客画面に出ない | 手順5（Realtime）が未設定 |
| AIが「担当者に接続しています。」しか返さない | `GEMINI_API_KEY` が未設定、または無料枠を使い切った |
| ログインできない | 手順7の「Auto Confirm User」が未チェック |
| スタイルが当たらない | `tailwind.config.ts` 変更後に dev を再起動していない |
| ページが500・404になる | `npx next dev` と `npx next build` を同時に走らせた（→[9章](#9-触ると壊れる箇所)） |
