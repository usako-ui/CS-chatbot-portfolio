...
| **提出後に店じまいする** | [10-2. 講座提出後の店じまい手順](#10-2-講座提出後の店じまい手順) | 5分 |
...
| `LIVE_CHAT_CLOSED` | `1` で本番チャットを案内ページに切り替える（任意・既定は未設定） | サーバー専用 |
...
提出後は Gemini の無料枠を守るためAPIキーを外し、Supabase も残さない運用を想定しています。
**体験デモ（`/demo-ec`）だけは動く状態で残します。**

```mermaid
flowchart TD
    A["講師の確認完了"] --> B["Vercel: GEMINI_API_KEY を削除"]
    B --> C["Vercel: LIVE_CHAT_CLOSED=1 を追加"]
    C --> D["Redeploy（必須）"]
    D --> E["本番チャット = 案内ページ<br/>体験デモ = 動く"]
    E --> F["Supabase プロジェクトを削除"]
    F --> G["デモはFAQスナップショットで継続"]

    X["NEXT_PUBLIC_SUPABASE_URL<br/>NEXT_PUBLIC_SUPABASE_ANON_KEY"] -.->|"消さないこと"| D

    style D fill:#ffe8b3,stroke:#d9a441
    style X fill:#ffd6d6,stroke:#a13d2d
```

| # | 操作 | 理由 |
|---|---|---|
| 1 | Vercel の環境変数から `GEMINI_API_KEY` を削除 | 無料枠を消費されないようにする |
| 2 | Vercel の環境変数に `LIVE_CHAT_CLOSED=1` を追加 | 本番チャットを案内ページに切り替える |
| 3 | **Redeploy する** | ⚠️ **環境変数の変更は既存のデプロイに反映されない。** ここを飛ばすと旧デプロイが古いキーで動き続ける |
| 4 | Supabase プロジェクトを削除（任意） | デモはスナップショットへ倒れるので動き続ける |
| 5 | Google AI Studio でAPIキーを削除（任意・推奨） | `.env.local` やシェル履歴に残った値を無効化する |

> ⚠️ **`NEXT_PUBLIC_SUPABASE_URL` と `NEXT_PUBLIC_SUPABASE_ANON_KEY` は消さないでください。**
> `lib/env.ts` の `publicEnv` がモジュール読み込み時に検証しており、
> 未設定だと**紹介ページを含む全ページが500になります**。
> Supabaseプロジェクトを消しても、この2つの環境変数は残したままにしてください
> （値が無効でもアプリは起動します）。
...
|---|---|
| `/`（紹介ページ） | そのまま動く |
| 本番チャット | **「このデモは終了しました」の案内ページ**。チャットは起動しないのでDBもGeminiも触らない |
| `/demo-ec` | **そのまま動く。** 体験者自身のAPIキーで、FAQはスナップショットから読む |
| `/login` `/dashboard` ほか | Supabase削除後はログインできなくなる（想定どおり） |

### 元に戻す場合

`LIVE_CHAT_CLOSED` を削除して再デプロイすれば本番チャットが戻ります。
Supabaseを消している場合は、`supabase/seed.sql` でFAQを流し直し、
`requirements.md` のDDLでテーブルとRLSを作り直す必要があります。

### デモがSupabaseに依存しない仕組み

`/demo-ec` はDBに書き込みませんが、**AIに渡すFAQ本文はDBから読んでいます。**
そのままではSupabase削除でデモも止まるため、`getActiveFaqsForDemo()` は
取得に失敗したら `lib/faqSnapshot.ts`（生成物・FAQ18件）へ倒します。

**本番の顧客チャット（`getActiveFaqs()`）はこの回避をしません。**
実データと食い違ったまま顧客に答えるほうが害が大きいためです。

> FAQを追加・変更したらスナップショットも再生成してください。
> 生成日時点の内容で固定されます。
